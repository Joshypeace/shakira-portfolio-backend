...
db.get(`SELECT * FROM appointments WHERE date = ?`, [date],async (err,row) =>{
  ...
  if(row) return res.status(400).json({ success: false, message: "Date already booked" });
});
  ...
  try {
      ...
      if (text.includes("Message successfully sent")) {
        // Save to DB
        db.run(`INSERT INTO appointments (name, email, phone, service, date, message)
                VALUES (?, ?, ?, ?, ?, ?)`,
          [name, email, phone, service, date, message], (err) => {
            if (err) {
        ...
        res.status(500).json({ success: false, message: "Failed to send WhatsApp message", apiResponse: text });
      }
    } catch (error) {
